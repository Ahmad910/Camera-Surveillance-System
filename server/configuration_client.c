...
static void make_request()
{
  ...
  if (connect(sockfd, (struct sockaddr*)&serv_addr, sizeof(serv_addr)) < 0) {
    ...
    int res = 1;
    while(res != 0){
       res = read(sockfd, msg, BUFSZ-1);
      if(res < 0) {
        perror("ERROR reading from motion server");
      } else {
        msg[res]='\0'; /* ensure msg is null terminated */
        int64_t ts = *((int64_t*)msg);
        int32_t mode = *((int32_t*)(msg+8));
        int32_t fsize = *((int32_t*)(msg+12));
        printf("simple_tcp_client: (res %d). Timestamp: %lld, mode: %d, frame size: %d\n", res, ts, mode, fsize);
      }
    }
    socket_close();
  }
